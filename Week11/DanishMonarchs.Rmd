...
author: "Emilie Hjortlund Bærnthsen"
date: "7 March 2025"
...
read.csv("data/REAL_Danish_Kings.csv")
```
...
kings1 <- read.csv("data/REAL_Danish_Kings.csv")
glimpse(kings1)
head(kings1)

kings2 <- read_csv("data/REAL_Danish_Kings.csv")
glimpse(kings2)
kings2

kings3 <- read.csv2("data/REAL_Danish_Kings.csv")
glimpse(kings3)

kings4 <- read_csv2( "data/REAL_Danish_Kings.csv")
glimpse(kings4)
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
Type 2 is a tibble and therefore 'tidyverse' functions.

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
I get a tibble/"tbl" function, wich is the function that tidyverse uses. That is what we want.

3. How many columns does the object have when created with these different functions?
There are 11 columns in kings1 an kings2. There is only 1 column in kings3 and kings4
